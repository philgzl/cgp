...
def cgp(x, y, fs, _discard_last_frame=False):
    ...
    gauss_kernel = utils.gauss_kernel(cfg.n_channels, cfg.spec_seg_len,
                                      cfg.spec_seg_hop, cfg.gauss_kernel_width)
    scores = utils.calc_tf_scores(x, y, cfg.spec_seg_len, cfg.spec_seg_hop,
                                  gauss_kernel)

    env_rms = (x ** 2).mean(axis=1, keepdims=True) ** 0.5
    env_ratio_dB = 20 * np.log10(x / env_rms)
    glimpse_thrs = np.interp(np.arange(cfg.n_channels), cfg.thr_channels,
                             cfg.glimpse_thrs)
