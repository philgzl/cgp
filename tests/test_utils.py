...
def test_remove_silent_frames(eng, rng, n, silence_idx):
    ...
    x_python, y_python = cgp.utils.remove_silent_frames(
        x, y, cfg.vad_win_len, cfg.vad_hop_len, cfg.vad_dyn_range,
...
def test_gen_cochlear_fb(eng):
    ...
    fb_matlab = eng.gen_cochlear_fb(
        float(cfg.fs),
        float(cfg.stft_n_fft),
        matlab.double(cfg.fcs.tolist())
    )
...
def test_gen_tmp_mod_lpf(eng):
    ...
    lpf_matlab = eng.gen_tmp_mod_LPF(
        float(cfg.lpf_fc),
        float(stft_fs),
        float(n_fft)
    )
...
def test_calc_tf_scores(eng, n):
    ...
    gauss_kernel = np.exp(- 0.5 * (
        np.subtract.outer(jj, spec_seg_idx) / cfg.gauss_kernel_width
    ) ** 2)
